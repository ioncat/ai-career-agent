...
Scan the full CV draft for repeated terms, phrases, or word combinations:

1. List every key term that appears 3+ times across all role descriptions (e.g., "release planning", "backlog", "requirements")
2. Flag terms that appear in adjacent sentences or same paragraph with similar meaning — these read as padding
3. For flagged repetitions → add to ⚠️ section with suggested variation or removal

Common patterns to catch:
- Same verb ("owned", "drove", "managed") starting multiple bullets in same block
- Same noun cluster ("release planning", "milestone tracking") in 3+ role descriptions
...
- 📣 `extra` — tool in CV, not in JD (neutral signal — shows working method, do not remove)
- ⚠️ `implied` — JD says "analytics" / "data-driven" / "A/B testing" generically but names no tool — note the category; if CV has a tool in that category → show it; if absent → add to 🔧
