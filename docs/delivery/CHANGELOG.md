...
- **DB cleanup — deleted 47 orphaned `user_id IS NULL` vacancy rows**: full audit of `db/agent.db` (654 rows) surfaced several data-hygiene gaps. Acted on the safe one: 47 rows dated 2026-05-29 (day 1, pre-EPIC-13 multi-user), `markdown_path` pointing at the defunct `callback-cv`/`job-board-monitor` predecessor projects — invisible to Flutter (every query filters by `user_id`), zero real value, no `duplicate_of` referencing them (verified before deleting, would have blocked the FK). This includes vacancy #10, reported same session as incorrectly showing in "Analyzed" — it wasn't a `stage()` bug, the row was dead data (empty `analysis_json`, source file gone). `db/agent.db` backed up first. **Deliberately did NOT harden `vacancies.user_id` to `NOT NULL`**: found `ON DELETE SET NULL` (EPIC-13) is intentional — vacancies survive as orphans if their user is ever deleted rather than cascade-deleting; a NOT NULL constraint would break that design. 654→607 rows. Four more findings from the same audit recorded in BACKLOG (fake-analyzed rows exact count 64/135, content_hash coverage 18%, legacy status count now 344) — not yet acted on.
