...
### Mass Action — explicit toggle button instead of long-press-only entry (added 2026-07-25)
**What:** multi-select mode is currently entered only via long-press on a card (`_enterMultiSelect`, `vacancy_inbox_screen.dart`) — user found this unintuitive on desktop ("без прочтения документации никогда не додумаешься"). Add a visible toggle button/icon near the filter controls in `InboxListHeader` that enters multi-select directly; hide `InboxFilterPanel` while active (not useful mid-selection).
**Why:** long-press has no desktop discovery convention — it's a good, intuitive pattern for touch/mobile (Flutter's same codebase targets mobile later per README), so it should stay, not be removed or platform-gated. On desktop it just shouldn't be the *only* path.
**Decided (2026-07-25):** keep long-press as-is (zero cost, future-proofs mobile); add the header button as the primary discoverable path on desktop. Open question carried into implementation: does entering mass-action mode also hide the search bar, or just the filter panel (search may still be useful to narrow the selection pool first)?
**Scope (not started):** icon button in `InboxListHeader`'s already-crowded action row — mind the same narrow-width overflow risk as the panel-width ticket above; may want to sequence after that ticket lands.
