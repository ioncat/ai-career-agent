...
**Part 2 — legacy statuses (344 rows, real debt):** `fetching`(264), `queued`(35), `new`(30), `done`(15) are outside the current state machine. Since the 5-stage taxonomy shipped (2026-07-16) these ARE visible again (mapped into "Inbox" by `core/vacancy_stage.py`'s legacy-status fallback) — no longer silently hidden, but still cluttering Inbox.
  ...
  - **Existing 264 rows — still open, user's call**: bulk-skip via the "Batch Analysis Mode" ticket's Skip N action once shipped, per user's stated plan. Note: with the fix above, these could ALSO just be reset to `queued` for automatic re-fetch on next RSS poll (cheap, low-risk, matches what the retry logic would have done anyway) — an alternative to skip-and-archive if the user would rather see them actually resolve. Not done — awaiting direction.
  - Then backfill their `published_at` too (Part 1's scope, once resolved).
