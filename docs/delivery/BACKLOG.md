...
**Part 3 — fake "analyzed" rows (found 2026-07-16, vacancy #10):** 64 of 135 rows with status ≥ analyzed have EMPTY `analysis_json` (no p1/p2) — the status lies, real analysis never ran or was lost. NOT caught by a legacy-status filter (status is a valid current value) — needs a separate check: `status IN (analyzed, ...) AND (analysis_json empty OR missing p1)`. Root cause likely early-project import artifacts (e.g. #10 is from 2026-05-29, day 1, `markdown_path` pointed to the old defunct `callback-cv` predecessor project — file doesn't exist on disk anymore; #10 was one of the Part 4 orphans, resolved by deletion, not reset).
  - Fix (remaining, non-orphan rows): reset to `fetched` (if `markdown_path` file still exists → re-analyzable) or `declined`/purge (if source file gone).
...
**Part 5 — `content_hash` coverage gap (found 2026-07-16):** 82% of rows (536/654) have `content_hash IS NULL` — EPIC-26 dedup detection only effectively covers ~18% of the DB. Backfillable by re-reading `markdown_path` (JD.md) and computing the hash for rows where the file still exists on disk (~577/581 non-orphan rows). Lower priority — dedup works going forward regardless; this only recovers detection for old rows. Not started.
