...
### Wire Critical Blocker pre-filter into an automatic trigger (added 2026-07-17, scope split 2026-07-23)
**What:** `POST /api/vacancies/{id}/prefilter` + Flutter "Check blockers" button ship manual-trigger-only (EPIC-27, delivered 2026-07-17) — deliberate, so the prompt/`## Critical Blockers` format could be validated before spending LLM calls automatically on every vacancy. This ticket is the follow-up: decide and wire an actual automatic trigger point once that validation is done.
...
**⚠️ Split 2026-07-23 — Stage 1 (title/domain) is now deterministic and free, Stage 2 (content) is still LLM-based.** `cv_prefilter()` now runs `_check_title_domain_signals()` + `_check_title_allowlist()` in plain Python BEFORE ever calling the LLM (see CHANGELOG 2026-07-23) — zero tokens, zero latency, already validated (827 tests + a 50-vacancy audit against real `db/agent.db` titles). The original design below was written when the ENTIRE pre-filter was one LLM call, so "auto-trigger" meant "spend tokens on every vacancy automatically" — that's no longer true for Stage 1. **Stage 1 can be wired to run automatically with none of the original cost/reliability hesitation** (see EPIC-27's Rollout section for why manual-only was chosen in the first place — that reasoning applies to Stage 2, not Stage 1). Stage 2 (content-based blockers: domain body-signal, ab_testing, payments, etc. — anything actually needing the LLM) stays manual-trigger-only until `gemma4:e4b` validation (separate P1 ticket above) or a provider the user trusts for unattended runs is confirmed.
**Scope (Stage 1 only, next up) — ~1–1.5h, both insertion points already located:**
- [ ] `tools/cv_prefilter.py` — small shared helper `apply_title_stage(vacancy_id, title)` wrapping the existing `_check_title_domain_signals()`/`_check_title_allowlist()` + `database.set_vacancy_blocker()`, so the RSS and import-jd call sites don't duplicate the write logic. ~15–20 min.
- [ ] `core/rss_watcher.py:245-248` — call it right after `update_vacancy_status(vacancy_id, "fetched")`; that exact spot already has a comment marking where the pre-filter auto-trigger decision was deferred on 2026-07-17. ~10 min.
- [ ] `web/api.py:833` (`api_import_jd`, after `update_vacancy_fields`) — same call. ~10 min.
- [ ] Tests for both call sites (flagged title → `blocker_flag` set immediately; fitting title → left unset). ~30–40 min.
- **UI stays exactly as-is** — the existing `_PrefilterBanner`/inbox badge already renders whatever's in `blocker_flag`/`blocker_reasons` regardless of how it got there; no new UI needed, just an earlier write.
**Scope (Stage 2, deferred, original full-auto design below):** needs to cover the same two vacancy-creation paths.
**Design — ✅ resolved (user, 2026-07-17), still applies to Stage 2 only:** one Settings toggle, "Auto-analyze blockers: on/off", is a master switch for BOTH logic and UI — the two modes are mutually exclusive by design, never both active:
