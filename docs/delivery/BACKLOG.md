...
**Scope (Stage 1 only, next up):** call `_check_title_domain_signals()`/`_check_title_allowlist()` right after vacancy insert in BOTH `RSSWatcher._process()` and `POST /api/vacancies/import-jd` (same two paths as the original full-auto design below — an RSS-only hook would silently miss imported vacancies); write `blocker_flag`/`blocker_reasons` immediately if either flags a mismatch; leave `blocker_flag` unset otherwise so the manual "Check blockers" button (Stage 2) still has something to do. **UI stays exactly as-is** — the existing `_PrefilterBanner`/inbox badge already renders whatever's in `blocker_flag`/`blocker_reasons` regardless of how it got there; no new UI needed, just an earlier write.
