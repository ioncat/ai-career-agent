...
start "Web Tracker :8080"  /D "%ROOT%"               cmd /k "call .venv\Scripts\activate && uvicorn web.api:app --port 8080 --reload"
start "Job Monitor (RSS)"  /D "%ROOT%"               cmd /k "call .venv\Scripts\activate && python services\job-monitor\monitor.py"
start "PDF Service :8002"  /D "%ROOT%\services\pdf"  cmd /k "call ..\..\.venv\Scripts\activate && uvicorn app:app --port 8002"
start "JD Parser :8001"    /D "%ROOT%\services\parser" cmd /k "call ..\..\.venv\Scripts\activate && uvicorn app:app --port 8001"
